...
def init_db():
    ...
    Parameters:
    - app: The Flask application object. This object must have a configuration (`config`) attribute that includes 'MONGO_URL', 'USERS_DB_NAME', and 'USERS_COLLECTION_NAME' keys.
    ...
    Note:
    - It's important to ensure that the Flask app object is properly configured with 'MONGO_URL', 'USERS_DB_NAME', and 'USERS_COLLECTION_NAME' before calling this function.
    """
    try:
        mongo_client = MongoClient(os.getenv('MONGO_DB_URL'))  # Establish connection to MongoDB
        db = mongo_client[os.getenv('USERS_DB_NAME')]  # Select the specified database
        return db[os.getenv('USERS_COLLECTION_NAME')]  # Attach the collection to the app object
